...
| **validate** | _action_ | <ul><li><b>collection</b> <i>Struct</i> </li><li><b>query</b> <i>Map</i> </li></ul> |  |
...
| **enqueue_fast_track_checker** | _action_ | <ul><li><b>published_record</b> <i>Struct</i> </li></ul> |  |
...
| **bulk_validate** | _action_ | <ul><li><b>rows</b> <i>Term</i> </li></ul> | Validates multiple records using `Ash.bulk_create/3`.<br /><br />The `rows` can be any enumberable, where each item which will be used as `params` for<br />the `DataAggregator.Records.ValidatedRecord.validate/1` action. |
