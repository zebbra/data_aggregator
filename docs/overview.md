...
1.  **`:gbif_taxonomy`**: Looks up taxonomic names against the GBIF Backbone Taxonomy. *Crucially, this first step also initializes/resets the `EncodedRecord` based on the source `Record` data before applying its specific logic.*
...
*   **Tracking Mode:** Only the *changes* made during an update are stored, not the full record state for each version (`change_tracking_mode :changes_only`).
