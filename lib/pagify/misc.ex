defmodule Pagify.Misc do
  ...
  Example:
      ...
      iex> Pagify.Misc.map_merge(%{a: 1, b: %{c: 2}}, %{b: %{d: 3}})
      %{a: 1, b: %{c: 2, d: 3}}

  one level of maps without conflict
      iex> Pagify.Misc.map_merge(%{a: 1}, %{b: 2})
      %{a: 1, b: 2}

  two levels of maps without conflict
      iex> Pagify.Misc.map_merge(%{a: %{b: 1}}, %{a: %{c: 3}})
      %{a: %{b: 1, c: 3}}

  three levels of maps without conflict
      iex> Pagify.Misc.map_merge(%{a: %{b: %{c: 1}}}, %{a: %{b: %{d: 2}}})
      %{a: %{b: %{c: 1, d: 2}}}

  non-map value in left
      iex> Pagify.Misc.map_merge(%{a: 1}, %{a: %{b: 2}})
      %{a: %{b:  2}}

  non-map value in right
      iex> Pagify.Misc.map_merge(%{a: %{b: 1}}, %{a: 2})
      %{a: 2}

  non-map value in both
      iex> Pagify.Misc.map_merge(%{a: 1}, %{a: 2})
      %{a: 2}
  """
  ...
  Example:
      ...
      iex> Pagify.Misc.deep_merge(%{a: 1, b: %{c: 2}}, %{b: %{d: 3}})
      %{a: 1, b: %{c: 2, d: 3}}

  one level of maps without conflict

      iex> Pagify.Misc.deep_merge(%{a: 1}, %{b: 2})
      %{a: 1, b: 2}

  two levels of maps without conflict

      iex> Pagify.Misc.deep_merge(%{a: [%{b: 1}]}, %{a: [%{c: 3}]})
      %{a: [%{b: 1}, %{c: 3}]}

  three levels of maps without conflict

      iex> Pagify.Misc.deep_merge(%{a: %{b: %{c: 1}}}, %{a: %{b: %{d: 2}}})
      %{a: %{b: %{c: 1, d: 2}}}

  non-map value in left

      iex> Pagify.Misc.deep_merge(%{a: 1}, %{a: %{b: 2}})
      %{a: %{b:  2}}

  non-map value in right

      iex> Pagify.Misc.deep_merge(%{a: %{b: 1}}, %{a: 2})
      %{a: 2}

  non-map value in both

      iex> Pagify.Misc.deep_merge(%{a: 1}, %{a: 2})
      %{a: 2}

  map of list

      iex> Pagify.Misc.deep_merge(%{a: [1, 2]}, %{a: [2, 3]})
      %{a: [1, 2, 3]}

  map of list of map

      iex> Pagify.Misc.deep_merge(%{a: [%{b: 1}, %{c: 2}]}, %{a: [%{c: 3}, %{d: 4}]})
      %{a: [%{b: 1}, %{c: 3}, %{d: 4}]}

  map of different types

      iex> Pagify.Misc.deep_merge(%{a: [1, 2]}, %{a: %{b: 2}})
      %{a: %{b: 2}}
  ...
  ## Example
      ...
      iex> Pagify.Misc.remove_nil_values(%{a: 1, b: nil, c: 3})
      %{a: 1, c: 3}

      iex> Pagify.Misc.remove_nil_values(%{a: 1, b: %{c: nil, d: 4}})
      %{a: 1, b: %{c: nil, d: 4}}
  """
  ...
  defp maybe_load_default_scopes(opts) do
    if Keyword.has_key?(opts, :default_scopes) do
      ...
      opts = maybe_put_compiled_pagify_scopes(resource, opts)
