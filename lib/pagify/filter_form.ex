defmodule Pagify.FilterForm do
  @moduledoc """
  ...
  > to `Pagify.FilterForm.new/2` to enforce a specific structure for the form and then merge
  > in the params.

  ```elixir
  # Create a FilterForm
  filter_form = Pagify.FilterForm.new(MyApp.Payroll.Employee)
  ```
  ...
  filter_form = Pagify.FilterForm.add_predicate(filter_form, :some_field, :eq, "Some Value")

  # Add a group and another predicate to that group
  {filter_form, group_id} = Pagify.FilterForm.add_group(filter_form, operator: :or, return_id?: true)
  filter_form = Pagify.FilterForm.add_predicate(filter_form, :another, :eq, "Other", to: group_id)
  ```
  ...
  query = Pagify.FilterForm.filter!(MyApp.Payroll.Employee, filter_form)
  filtered_employees = MyApp.Payroll.read!(query)

  # Or use one of the other filter functions
  Pagify.FilterForm.to_filter_expression(filter_form)
  Pagify.FilterForm.to_filter_map(filter_form)
  ```
  ...
  defp filter_form_component(%{component: %{source: %Pagify.FilterForm{}}} = assigns) do
    ~H\"\"\"
  ...
  defp filter_form_component(
    ...
    <div class="flex flex-row gap-2 mt-4">
      <.input
        type="select"
        options={Pagify.FilterForm.fields(Employee)}
        field={@component[:field]}
      />
      <.input
        type="select"
        options={Pagify.FilterForm.predicates(Employee)}
  ...
  def mount(_params, _session, socket) do
    socket =
      socket
      |> assign(:filter_form, Pagify.FilterForm.new(Employee))
  ...
  def handle_event("filter_submit", %{"filter" => params}, socket) do
    ...
    case Pagify.FilterForm.filter(Employee, filter_form) do
      {:ok, query} ->
        {:noreply,
         socket
  ...
  def handle_event("remove_filter_component", %{"component-id" => component_id}, socket) do
    {:noreply,
     assign(socket,
       filter_form:
         Pagify.FilterForm.remove_component(socket.assigns.filter_form, component_id)
     )}
  end
  ...
  def handle_event("add_filter_group", %{"component-id" => component_id}, socket) do
    {:noreply,
     assign(socket,
       filter_form: Pagify.FilterForm.add_group(socket.assigns.filter_form, to: component_id)
     )}
  end
  ...
  def handle_event("add_filter_predicate", %{"component-id" => component_id}, socket) do
    {:noreply,
     assign(socket,
       filter_form:
         Pagify.FilterForm.add_predicate(socket.assigns.filter_form, :name, :contains, nil,
           to: component_id
         )
     )}
  end
  ...
  ```elixir
  filter = Pagify.FilterForm.to_filter_expression(form)
  ...
  defp do_add_group({%Pagify.FilterForm{id: id, name: parent_name} = form, i}, id, group) do
    name = parent_name <> "[components][#{i}]"
    %{form | components: form.components ++ [%{group | name: name}]}
  end

  defp do_add_group({%Pagify.FilterForm{} = form, _i}, id, group) do
    %{
      form
        ...
        case component do
          %Pagify.FilterForm{} ->
            to_form(component,
              ...
              id: component.id,
              errors: Pagify.FilterForm.errors(component)
            )
