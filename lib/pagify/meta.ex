defmodule Pagify.Meta do
  ...
  - `:opts` - The options passed to the `Pagify` struct.
  ...
  @doc """
  Returns a `Pagify.Meta` struct with the given params, errors, and opts.
  ...
  `Pagify.validate/2`, which returns a `Pagify` struct on success. You can then pass
  ...
  parameters and the opts, which both are needed to call this function.
      ...
      def list_posts(%{} = params) do
        ...
        with {:ok, %Pagify{} = pagify} <- Pagify.validate(Post, params, opts),
             {:ok, %Pagify{} = pagify} <- custom_validation(pagify, params, opts) do
          Pagify.run(Post, pagify, opts)
        end
      end

  In your custom validation function, you can retrieve and manipulate the filter
  values in the `Pagify` struct.

      defp custom_validation(%Pagify{} = pagify, %{} = params, opts) do
        filters = pagify.filters

        if Keyword.get(filters, :name) != nil do
          errors = [filters: [%Ash.Error.Query.InvalidFilterReference{field: :name}]]
          {:error, Pagify.Meta.with_errors(params, errors, opts)}
        else
          {:ok, pagify}
        end
      end
