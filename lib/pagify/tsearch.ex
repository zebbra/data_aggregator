defmodule Pagify.Tsearch do
  @moduledoc """
  Pagify full-text search utilities.

  This module provides utilities for full-text search in Pagify. For a basic usage
  you can `use Pagify.Tsearch` and implement the `tsvector` calculation in your
  resource.
  ...
  Pagify.Tsearch provides the following calculations which are used by `Pagify.search` for full-text search:
  ...
  ```elixir
  defmodule MyApp.Resource do
    use Pagify.Tsearch, only: [:full_text_search, :full_text_search_rank]
    require Ash.Query
  ...
  `Pagify.validate_and_run/4` call (or other functions provided by Pagify). This approach is mandatory so we
  can serialize the custom tsvector in `Pagify.query_to_filters_map` and restore it in `Pagify.query_for_filters_map`
  accordingly.
  ...
  def search(query, opts \\\\ []) do
    opts = Pagify.set_tsvector(:custom_tsvector, opts)
    query
    |> Pagify.validate_and_run(MyApp.Resource, opts)
  end
  ```
  """
