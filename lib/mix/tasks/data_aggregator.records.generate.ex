defmodule Mix.Tasks.DataAggregator.Records.Generate do
  ...
  * `--count` - The number of records to generate (default: #{@default_count})
  ...
  def run(args) do
    ...
    count
    |> generate_records(invalid / 100.0)
    |> CSV.encode(headers: true)
    |> Stream.into(IO.stream())
    |> Stream.run()
  end

  defp generate_records(count, invalid_rate) do
    Stream.map(1..count, &generate_record(&1, invalid_rate))
  end

  defp generate_record(index, invalid_rate) do
    if invalid_rate > :rand.uniform() do
  ...
  defp invalid_record(index) do
    %{
      "Numéro scientifique GBIF" => nil,
      "Scientific Name" => "Invalid #{index}",
      "Age" => "",
      "Auteur et date ssp" => "Invalid, 1831",
      "Autres numéros" => "",
      "Collecteur" => "Dr Gosse",
      "DAYCOLLECTED" => "",
      "ENDOFPERIODDAY" => "",
