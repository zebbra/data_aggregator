defmodule DataAggregator.Records.Collection.Actions.SendValidation do
  @moduledoc """
  ...
  def run(input, _opts, %{tenant: tenant} = ctx) do
    ...
    query
    |> stream_query()
    |> set_validation_status(:validating, validation, ctx)
    ...
    |> set_validation_status(:in_validation, validation, ctx)
  ...
  defp set_validation_status(stream, status, validation, %{actor: actor, tenant: tenant}) do
    max_concurrency = Records.import_max_concurrency()
    batch_size = ceil(Records.import_batch_size() / max_concurrency)

    stream_params =
      if validation.channel == :fast_track do
        Enum.map(stream, &%{id: &1.record_id})
      else
        stream
      end

    Ash.bulk_update(stream_params, :update_validation_status, %{status: status},
      actor: actor,
      authorize?: false,
      domain: Records,
      resource: Record,
