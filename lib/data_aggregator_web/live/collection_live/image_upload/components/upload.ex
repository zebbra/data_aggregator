defmodule DataAggregatorWeb.CollectionLive.ImageUpload.Components.Upload do
  @moduledoc """
          ...
          <.fieldset>
            <.fieldgroup>
              <.collapsible_notification
                :if={@error_message}
                title={~t"An error has occured"m}
                color="red"
              >
                <:action>
                  <%= ~t"Show more"m %>
  ...
  defp handle_upload_error(error) do
    case error do
      ...
      error ->
        {:postpone, {:error, "Could not create image upload file: #{inspect(error)}"}}
    end
  end
  ...
  defp handle_flash(socket, image_upload) when is_nil(image_upload) do
