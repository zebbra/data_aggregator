defmodule DataAggregatorWeb.Components.ThemeSelect do
  @moduledoc """
  ...
  def render(assigns) do
    ...
    <button
      id={@id}
      class="btn btn-ghost btn-circle text-base-content/75 hover:text-base-content"
  ...
  defp icon_class(theme, current) do
    class = if theme == current, do: "opacity-100", else: "opacity-0 rotate-90"
    ["h-6 w-6", class]
  end
  ...
  defmacro __using__(_opts) do
    quote do
      import DataAggregatorWeb.Components.ThemeSelect, only: [theme_select: 1]
    end
  end
end
